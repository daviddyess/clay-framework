...
class styles {
	...
	private static function themeOverride($application,$file){
		return file_exists(THEMES_PATH.THEME.'apps/'.$application.'/styles/'.$file);
	}
		...
		switch($args['type']){
			case('app'):
				if(self::themeOverride($args['name'],$args['file'])){
					self::$map['styles'][$pattern]['src'] = REL_THEMES_PATH.THEME.'apps/'.$args['name'].'/styles/'.$args['file'];
	...
	private static function processInternal($args){
		...
		switch($args['type']){
			case('app'):
				if(self::themeOverride($args['name'],$args['file'])){
					self::$map['styles'][$pattern] = THEMES_PATH.THEME.'apps/'.$args['name'].'/styles/'.$args['file'];
				} elseif(self::inApplication($args['name'],$args['file'])) {
